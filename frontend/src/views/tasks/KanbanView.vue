...
import {TASK_PRIORITY, TASK_STATUS} from "@/consts/task.js";
  ...
  methods: {
    ...
    getTasksByStatus(statusId) {
     ...
     getPriorityColor(priorityId) {
       priority = priorities.value.find(p => p.id === priorityId)
      return priority?.color || '#CBD5E0'
    },

     getPriorityTextColor(priorityId) {
       priority = priorities.value.find(p => p.id === priorityId)
     ...
     getStatusColor(statusId) {
       status = statuses.value.find(s => s.id === statusId)
...
// Drag & Drop
    ...
    handleDrop(event, newStatus) {
      ...
      taskIndex = tasks.value.findIndex(t => t.id === draggedTask.value.id)
      if (taskIndex !== -1) {
